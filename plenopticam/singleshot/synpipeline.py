...
class SynthesisPipeline(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.I1 is None:
            raise RuntimeError("Cannot create synthesis pipeline without input image!")
        self.u0 = kwargs['u0'] if 'u0' in kwargs else (self.I1.shape[0]//2, self.I1.shape[1]//2)

    def generate_synthesized_view(self):
        x1A, y1A, __ = np.indices(self.I1.shape)
        H, W, C = self.I1.shape

        x1B = self.D1A*(self.D0-self.t)*x1A/(self.D0*(self.D1A-self.t)) + self.t*(self.D1A-self.D0)*u0[0]/(self.D0*(self.D1A-self.t))
        x1B = x1B.astype(np.uint16).transpose(2,0,1)
        y1B = self.D1A*(self.D0-self.t)*y1A/(self.D0*(self.D1A-self.t)) + self.t*(self.D1A-self.D0)*u0[1]/(self.D0*(self.D1A-self.t))
